...
public class QueryController {
	...
	private final String QUERY_TYPES[] = { "SELECT", "INSERT", "DELETE", "DDL", "UPDATE", "CALL", "COMMIT", "ROLLBACK",
			"DESCRIBE" };

	@RequestMapping(value = "/query", method = RequestMethod.GET)
	public String worksheet(Model model, HttpServletResponse response, HttpServletRequest request, HttpSession session)
			throws Exception {
	...
	public String worksheetAction(Model model, @RequestParam(value = "query", required = true) String query,
			...
			@RequestParam(value = "explainPlan", required = true) String explainPlan, HttpServletResponse response,
		...
		if (query.trim().length() != 0) {
			if (splitQueryStr.length == 1) {
				...
				if (determineQueryType(s).equals("SELECT")) {
					try {
						if (explainPlan.equals("Y")) {
							...
							webResult = genericDAO.runGenericQuery("explain " + s, null,
									(String) session.getAttribute("user_key"), -1);
							model.addAttribute("explainresult", webResult);
							model.addAttribute("query", s);
						} else {
							long start = System.currentTimeMillis();
							webResult = genericDAO.runGenericQuery(s, null, (String) session.getAttribute("user_key"),
						...
						result.setCommand(s);
						result.setMessage(
								ex.getMessage() == null ? "ERROR: Unable to run query" : "ERROR: " + ex.getMessage());
						result.setRows(-1);
						model.addAttribute("result", result);
						model.addAttribute("query", s);
					...
					if (s.length() > 0) {
						if (determineQueryType(s).equals("COMMIT")) {
							result = genericDAO.runStatement("commit", elapsedTime, "Y",
							...
							result = genericDAO.runStatement("rollback", elapsedTime, "Y",
									(String) session.getAttribute("user_key"));
							...
							result = genericDAO.runStatement(s, elapsedTime, "Y",
									(String) session.getAttribute("user_key"));
							model.addAttribute("result", result);
							if (result.getMessage().startsWith("SUCCESS")) {
								addCommandToHistory(session, userPrefs, s);
							...
							try {
								webResult = genericDAO.runGenericQuery(s, null,
										(String) session.getAttribute("user_key"), -1);
								model.addAttribute("queryResults", webResult);
								model.addAttribute("query", s);
								model.addAttribute("querysql", s);
								...
								result.setCommand(s);
								result.setMessage(ex.getMessage() == null ? "ERROR: Unable to run query"
										: "ERROR: " + ex.getMessage());
								result.setRows(-1);
								model.addAttribute("result", result);
								model.addAttribute("query", s);
							...
							result = genericDAO.runStatement(s, elapsedTime, "N",
									(String) session.getAttribute("user_key"));
							...
							if (result.getMessage().startsWith("SUCCESS")) {
								...
								session.setAttribute("schemaMap",
										genericDAO.populateSchemaMap((String) session.getAttribute("schema"),
												(String) session.getAttribute("user_key")));
							}
						}
				...
				SortedMap<String, Object> queryResults = handleMultipleStatements(splitQueryStr, userPrefs, queryCount,
						elapsedTime, explainPlan, session);
	...
	public String fileuploadRequest(Model model, @RequestParam("file") MultipartFile file, HttpServletRequest request)
			throws Exception {
	...
	public String executeQuery(Model model, HttpServletResponse response, HttpServletRequest request,
		...
		try {
			WebResult webResult = genericDAO.runGenericQuery(query, null, (String) session.getAttribute("user_key"),
					userPrefs.getMaxRecordsinSQLQueryWindow());
	...
	private SortedMap<String, Object> handleMultipleStatements(String[] splitQueryStr, UserPref userPrefs,
			String queryCount, String elapsedTime, String explainPlan, HttpSession session)
			throws SQLException, PivotalMySQLWebException {
		int counter = 9000;
		boolean ddl = false;
			...
			if (determineQueryType(s).equals("SELECT") || determineQueryType(s).equals("DESCRIBE")) {
				...
				if (s.length() > 0) {
					if (determineQueryType(s).equals("COMMIT")) {
						...
						result = genericDAO.runStatement(s, elapsedTime, isDDL(s),
								(String) session.getAttribute("user_key"));
					}
		...
		if (ddl == true) {
			session.setAttribute("schemaMap", genericDAO.populateSchemaMap((String) session.getAttribute("schema"),
					(String) session.getAttribute("user_key")));
		}
	...
	private String isDDL(String s) {
		if (determineQueryType(s).equals("INSERT") || determineQueryType(s).equals("UPDATE")
