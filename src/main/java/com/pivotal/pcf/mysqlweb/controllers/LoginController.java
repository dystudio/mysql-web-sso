...
public class LoginController {
	...
	public String login(Model model, HttpSession session, OAuth2Authentication auth) throws Exception {
		...
		if (jsonString != null) {
			if (jsonString.length() > 0) {
				try {
					...
					MysqlConnection newConn = new MysqlConnection(login.getUrl(), new java.util.Date().toString(),
							login.getUsername().toUpperCase());
					...
					databaseList = genericDAO.runGenericQuery(Constants.DATABASE_LIST, null, session.getId(), -1);
	...
	public String login(@RequestParam(value = "username", required = true) String username,
			...
			@RequestParam(value = "url", required = true) String url, Model model, HttpSession session)
			throws Exception {
		...
		try {
			...
			MysqlConnection newConn = new MysqlConnection(url, new java.util.Date().toString(), username.toUpperCase());

			cm.addConnection(newConn, session.getId());
			cm.addDataSourceConnection(AdminUtil.newSingleConnectionDataSource(url, username, password),
					session.getId());
